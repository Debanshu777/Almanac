...
class CalenderViewModel : ViewModel() {
    ...
    fun setSelectedDateFromSelectedYear(year: Int) {
        viewModelScope.launch {
            val dateCalculation =
                if (selectedDate.value!!.dayOfMonth >= 10) (selectedDate.value!!.dayOfMonth).toString() else ("0${selectedDate.value!!.dayOfMonth}")
            val monthCalculation =
                if (selectedDate.value!!.monthValue >= 10) (selectedDate.value!!.monthValue).toString() else ("0${selectedDate.value!!.monthValue}")
            val yearToDate = "$year-${monthCalculation}-$dateCalculation"
            selectedDate.value =
                LocalDate.parse(yearToDate)
            formatDateString.value =
                selectedDate.value!!.format(CalenderUtils.dayMonthYearFormatter)
        }
    }
